/******************************************************************************
 * kvm selfballoon driver (and optional frontswap self-shrinking driver)
 *
 ...
 * support for kvm Transcendent Memory ("tmem").  The policy it implements
 ...
 * such as kvm, RAM is often a scarce resource and we would like to ensure
 ...
 *
 * "Selfballooning" creates memory pressure by managing the kvm balloon
 ...
 * kernel RAM and, presumably using cleancache, into kvm tmem where
 * kvm can more efficiently optimize RAM utilization for such pages.
 *
 * When kernel memory demand unexpectedly increases faster than kvm, via
 ...
 * to absorb this swapping into kvm tmem.  However, due to the fact
 ...
 *
 * Selfballooning is disallowed in kvm host.
 *
 */
...
static int kvm_selfballooning_enabled __read_mostly;
/*
...
static int __init kvm_nofrontswap_selfshrink_setup(char *s)
{
	use_frontswap_selfshrink = false;
	return 1;
}

__setup("noselfshrink", kvm_nofrontswap_selfshrink_setup);

/* Disable with kernel boot option. */
static bool use_selfballooning __initdata = true;

static int __init kvm_noselfballooning_setup(char *s)
{
	use_selfballooning = false;
	return 1;
}

__setup("noselfballooning", kvm_noselfballooning_setup);
#else /* !CONFIG_FRONTSWAP */
/* Enable with kernel boot option. */
static bool use_selfballooning __initdata = false;

static int __init kvm_selfballooning_setup(char *s)
{
	use_selfballooning = true;
	return 1;
}

__setup("selfballooning", kvm_selfballooning_setup);
...
int register_kvm_selfballooning(struct sys_device *sysdev)
{
	int error = -1;
...
EXPORT_SYMBOL(register_kvm_selfballooning);

static int __init kvm_selfballoon_init(void)
{
	bool enable = false;

	kvm_selfballooning_enabled = use_selfballooning;
	if (kvm_selfballooning_enabled) {
		pr_info("kvm/balloon: Initializing kvm "
	...
	if (frontswap_selfshrinking) {
		pr_info("kvm/balloon: Initializing frontswap "
...
subsys_initcall(kvm_selfballoon_init);
